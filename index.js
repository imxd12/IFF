...
(function () {
    ...
    // ----------------------------------------------------
    // WEATHER, GREETINGS & VOICE CONTROLS UPGRADE
    // ----------------------------------------------------
    let currentWeatherData = null;

    async function initWeather() {
        const tempEl = document.getElementById('weatherTempDisplay');
        const condEl = document.getElementById('weatherConditionDisplay');
        const cityEl = document.getElementById('weatherCityDisplay');
        const humidEl = document.getElementById('weatherHumidityDisplay');
        const windEl = document.getElementById('weatherWindDisplay');
        const updateTimeEl = document.getElementById('weatherUpdateTime');
        const badgeText = document.getElementById('weatherTextShort');
        const badgeIcon = document.getElementById('weatherIcon');
        ...
        const setWeatherUI = (weather) => {
            ...
            if (condEl) condEl.textContent = `${details.emoji} ${details.condition}`;
            ...
            if (badgeText) badgeText.textContent = `${temp}°C • ${details.condition}`;
            
            if (badgeIcon && window.lucide) {
                badgeIcon.setAttribute('data-lucide', details.icon);
                badgeIcon.className = `w-4 h-4 ${details.color} ${details.animation}`;
                window.lucide.createIcons({ root: badgeIcon.parentNode });
            }

            // Instantly refresh greeting text to apply smart emoji
            updateTextGreeting();
        };
        ...
        const getWeatherDetails = (code) => {
            ...
            return { condition, icon, emoji, color, animation };
        };
        ...
        const fetchWeather = async () => {
            ...
            try {
                    ...
                    const browserLoc = await new Promise((resolve) => {
                        if (navigator.geolocation) {
                            navigator.geolocation.getCurrentPosition(
                                p => resolve({ lat: p.coords.latitude, lon: p.coords.longitude, city: 'Local Area' }),
                                () => resolve(null),
                                { timeout: 4000 }
                            );
                        } else {
                ...
                if (cached) {
                    ...
                    if (condEl) condEl.textContent = 'Weather Offline';
                    if (cityEl) cityEl.textContent = 'Network Error';
                    if (badgeText) badgeText.textContent = 'Weather Offline';
                }
            } finally {
        ...
        // Try Cache First (10 minute limit)
        const cached = localStorage.getItem(cacheKey);
        if (cached) {
            const parsed = JSON.parse(cached);
                ...
                if (window.showPromptModal) {
                    window.showPromptModal("Change Name", username, "Enter your name", (newName) => {
                        if (newName && newName.trim()) {
                            ...
                            localStorage.setItem('fin_userName', trimmedName);
                            localStorage.setItem('username', trimmedName); // compatibility
    ...
    function initVoiceControls() {
        ...
        const updateMuteUI = () => {
            const isMuted = localStorage.getItem('fin_voice_mute') === 'true';
            if (isMuted) {
                if (toggleText) toggleText.textContent = "Voice Muted";
                if (toggleIcon) {
                    toggleIcon.setAttribute('data-lucide', 'volume-x');
                    toggleIcon.className = "w-3.5 h-3.5 text-rose-400";
                    window.lucide.createIcons({ root: toggleIcon.parentNode });
                }
            } else {
                if (toggleText) toggleText.textContent = "Voice Active";
                if (toggleIcon) {
                    toggleIcon.setAttribute('data-lucide', 'volume-2');
                    toggleIcon.className = "w-3.5 h-3.5 text-emerald-400";
                    window.lucide.createIcons({ root: toggleIcon.parentNode });
                }
            }
        };
        ...
        if (muteToggle) {
            if (localStorage.getItem('fin_voice_mute') === null) {
                localStorage.setItem('fin_voice_mute', 'false');
            }
            updateMuteUI();
            ...
            muteToggle.addEventListener('click', (e) => {
                e.preventDefault();
                const isMuted = localStorage.getItem('fin_voice_mute') === 'true';
                localStorage.setItem('fin_voice_mute', !isMuted);
                updateMuteUI();
                showSnackbar(!isMuted ? "Voice greetings enabled! 🔊" : "Voice greetings muted! 🔇");
                
                if (isMuted) {
                    speakVoiceGreeting(true);
                } else {
                    window.speechSynthesis.cancel();
                }
            });
    ...
    function initOnboarding() {
        ...
        const input = document.getElementById('onboardingUsernameInput');
        const saveBtn = document.getElementById('saveOnboardingUsernameBtn');
        ...
        if (saveBtn && input && modal) {
            saveBtn.addEventListener('click', (e) => {
                ...
                if (val) {
                    localStorage.setItem('fin_userName', val);
                    ...
                    showSnackbar(`Welcome to MoneyFlow, ${val}! ✨`);
                } else {
                    input.focus();
